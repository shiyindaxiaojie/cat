/*
 * Copyright (c) 2011-2018, Meituan Dianping. All Rights Reserved.
 *
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements. See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 */
package com.dianping.cat.consumer.dump;

import java.lang.reflect.Field;
import java.util.concurrent.TimeUnit;

import org.junit.Assert;
import org.junit.Test;
import org.unidal.cat.message.storage.MessageDumper;
import org.unidal.cat.message.storage.MessageDumperManager;

import com.dianping.cat.message.spi.internal.DefaultMessageTree;

import io.netty.buffer.ByteBuf;
import io.netty.buffer.PooledByteBufAllocator;

public class DumpAnalyzerTest {
	@Test
	public void testOnlineSnapshotFlushesWithoutClosingCurrentHour() throws Exception {
		DumpAnalyzer analyzer = new DumpAnalyzer();
		MockMessageDumperManager manager = new MockMessageDumperManager();
		Field field = DumpAnalyzer.class.getDeclaredField("m_dumperManager");
		long startTime = TimeUnit.HOURS.toMillis(1234);

		field.setAccessible(true);
		field.set(analyzer, manager);
		analyzer.initialize(startTime, TimeUnit.HOURS.toMillis(1), 0);
		analyzer.doSnapshot();

		Assert.assertEquals(1234, manager.m_flushedHour);
		Assert.assertEquals(-1, manager.m_closedHour);
	}

	@Test
	public void testInvalidMessageReleasesBuffer() {
		DumpAnalyzer analyzer = new DumpAnalyzer();
		DefaultMessageTree tree = new DefaultMessageTree();
		ByteBuf buffer = PooledByteBufAllocator.DEFAULT.buffer(16);

		tree.setMessageId("invalid");
		tree.setBuffer(buffer);
		analyzer.process(tree);

		Assert.assertEquals(0, buffer.refCnt());
	}

	private static class MockMessageDumperManager implements MessageDumperManager {
		private int m_closedHour = -1;

		private int m_flushedHour = -1;

		@Override
		public void close(int hour) {
			m_closedHour = hour;
		}

		@Override
		public MessageDumper find(int hour) {
			return null;
		}

		@Override
		public MessageDumper findOrCreate(int hour) {
			return null;
		}

		@Override
		public void flush(int hour) {
			m_flushedHour = hour;
		}
	}
}
